...
## Model
* represents the `data` and `business logic` of the application - can be called the **`Business Layer`**
* defines the structure of the data, performs data manipulation, and interacts with data sources (databases , web services)

* Models also contain the following type of logics: Business logic, Data access logic, Validation logic
* _The `classes in Models` are called Entity classes or Domain classes_

## View
* responsible for presenting the `user interface` to the users - can be called `Display Layer`
* defines `how the data should be displayed` to the users and interacts with them
...
* include processes input data, interacts with model, and determines the appropriate view to render
...
* typically have a one-to-one relationship with a table in the database. 
* related to the data access layer (DAL) in the application
...
## With ViewBag
* ViewBag is a `dynamic property/object`; one of the properties of `"ControllerBase"` class 
...
* _The header and footer usually contain boilerplate markup for many metadata elements and links to script and style assets_ 
...
## Partial views 
* Represents a **`reusable component or portion of HTML markup`** 
